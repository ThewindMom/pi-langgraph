...
export function projectResult(threadId: string, state: WorkflowStateValue): CodingRunResult {
  if (state.status === "running") return projectApproval(threadId, state);
  ...
  const unresolvedRisks = unique([
    ...
    ...state.findings.flatMap((finding) => finding.risks),
    ...state.changes.flatMap((change) => change.unresolvedRisks),
  ]);
